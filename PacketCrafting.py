...
import struct
import random
from scapy import IP, TCP, sr1
from InputModule import ip_input, port_input


# # CREATION OF PACKET USING SCAPY
# ip = IP( dst = ip_input) # Creating an IP packet with the destination IP address from user input
# tcp = TCP(dport = port_input, flags='S') # Creating a TCP packet with the destination port from user input and SYN flag set to initiate the connection



# CREATION OF RAW PACKET USING SOCKETS AND STRUCT
...
def packet_crafting():
    print("Crafting packet...") # Indicating that the packet is being crafted
    ...
    protocol = socket.IPPROTO_TCP # Protocol number for TCP (6)
    ...
    return packet  # Returning the crafted packet